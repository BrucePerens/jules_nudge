...
def send_message(session_name, prompt, headers):
    ...
    try:
        ...
        response.raise_for_status()
        logging.info(f"https://jules.google.com/session/{session_name}: Sent message '{prompt}'.")
    except requests.exceptions.RequestException as e:
        logging.error(f"https://jules.google.com/session/{session_name}: Failed to send message. Error: {e}")
...
def approve_plan(session_name, headers):
    ...
    try:
        ...
        response.raise_for_status()
        logging.info(f"https://jules.google.com/session/{session_name}: Successfully approved plan.")
    except requests.exceptions.RequestException as e:
        logging.error(f"https://jules.google.com/session/{session_name}: Failed to approve plan. Error: {e}")
    ...
    match state:
        ...
        case "PAUSED" | "FAILED" | "STUCK":
            if "Jules encountered an error when cloning the repo" in str(session):
                logging.info(f"https://jules.google.com/session/{session_name}: Skipping 'Proceed' because a repo cloning error was detected.")
            else:
        ...
        case _:
            logging.warning(f"https://jules.google.com/session/{session_name} is in unexpected state '{state}'.")
